...
| TutorialProgress (FT-09) | ✅ Đã code, mở rộng 2026-08-10 | Complete/UncompleteStep + GetTutorialProgress, cộng thêm `POST /api/tutorials/{id}/complete` (gộp lưu Achievement + đánh giá độ khó `PerceivedDifficulty` ở bước cuối, chỉ lần hoàn thành đầu tiên) và `TutorialDetailDto` giờ có `RatingSummary`/`HasAchievement`/`HasRated`/tiến độ. Chi tiết field/entity xem `docs/Origami_ERD_Design_v3.md` mục 3, 6. **Known limitation:** endpoint `/complete` và toàn bộ luồng thưởng (skill points/Hạt Gấp/streak/quest/Achievement/rating) chưa phân biệt được tutorial đang bị FE tái dùng làm placement test (`TutorialDetailPage` với `mode=placement-test`) — placement test là một lượt làm bài (`PlacementTestAttempt` gắn `LearningPathLevel`), không phải thuộc tính của `Tutorial`, nên KHÔNG được gate bằng 1 cờ bool trên `Tutorial` (cùng 1 tutorial vẫn phải thưởng bình thường khi học ngoài placement test). Gate đúng sẽ cần thêm khi `PlacementTestAttempt` được thiết kế/implement — chưa làm ở đây, cần xác nhận riêng trước khi động vào. |
