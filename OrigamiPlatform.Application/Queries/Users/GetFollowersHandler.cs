...
public class GetFollowersHandler
{
    ...
    public GetFollowersHandler(IFollowRepository follows, ITutorialRepository tutorials)
    {
        _follows = follows;
        _tutorials = tutorials;
    }

    public async Task<PagedResult<FollowerUserDto>> HandleAsync(GetFollowersQuery query, CancellationToken ct = default)
    {
        var paged = await _follows.GetFollowersAsync(query.TargetUserId, query.Page, query.PageSize, ct);
