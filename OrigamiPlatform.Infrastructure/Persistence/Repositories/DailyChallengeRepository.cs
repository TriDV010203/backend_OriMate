...
public class DailyChallengeRepository : IDailyChallengeRepository
{
    ...
    public Task<DailyChallenge?> GetByDateAsync(DateOnly challengeDate, CancellationToken ct = default)
        => _db.DailyChallenges
    ...
    public Task<DailyChallenge?> GetByIdAsync(Guid id, CancellationToken ct = default)
        => _db.DailyChallenges
            .Include(c => c.Tutorial).ThenInclude(t => t.Author).ThenInclude(a => a.Profile)
            .FirstOrDefaultAsync(c => c.Id == id, ct);

    public async Task AddAsync(DailyChallenge challenge, CancellationToken ct = default)
    {
        _db.DailyChallenges.Add(challenge);
        await _db.SaveChangesAsync(ct);
    }

    public async Task UpdateAsync(DailyChallenge challenge, CancellationToken ct = default)
    {
        _db.DailyChallenges.Update(challenge);
        await _db.SaveChangesAsync(ct);
    }

    public async Task<HashSet<Guid>> GetRecentlyUsedTutorialIdsAsync(DateOnly sinceDate, CancellationToken ct = default)
        => (await _db.DailyChallenges
    ...
    public async Task<PagedResult<DailyChallenge>> GetAllForAdminAsync(
        DateOnly? fromDate,
        DateOnly? toDate,
        ...
        CancellationToken ct = default)
    {
        ...
        return new PagedResult<DailyChallenge>(items, totalCount, page, pageSize, totalPages);
    }

    public Task<int> CountByTutorialAuthorAsync(Guid authorUserId, CancellationToken ct = default)
        => _db.DailyChallenges.CountAsync(c => c.Tutorial.AuthorId == authorUserId, ct);
}
